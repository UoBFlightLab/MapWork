...
def compile_flight_path_south():
  # get relevant features
  sffs = list_all_features(safe_get_layer_by_name('safti'))
  tefs = list_all_features(safe_get_layer_by_name('testa'))
  svfs = list_all_features(safe_get_layer_by_name('survey_points'))
  hmfs = list_all_features(safe_get_layer_by_name('homer'))
  # make complete flight list
  wpfs = hmfs+tefs+sffs+svfs+tefs+hmfs
  fl_polyline = [f.geometry().asPoint() for f in wpfs]
...
def add_approach_markers_south():
  wpfs = list_all_features(safe_get_layer_by_name('survey_points'))
  sffs = list_all_features(safe_get_layer_by_name('safti'))
  gm_safti = sffs[0].geometry()
  gm_wp1 = wpfs[0].geometry()
  dist_safti_wp1 = gm_safti.distance(gm_wp1)
...
def make_flight_legs_south():
  # get relevant features and sort by the ID attribute
  wpfs = list_all_features(safe_get_layer_by_name('all_waypoints'))
  def sort_fn(f):
    return f.attribute('id')
  wpfs.sort(key=sort_fn)
  
  # make a new line layer
  layer = QgsVectorLayer('LineString?crs=epsg:27700', 'Flight Legs' , 'memory')
  prov = layer.dataProvider()
  
  # loop through each pair
  for ii in range(len(wpfs)-1):
    fl_polyline = [wpfs[ii].geometry().asPoint(),wpfs[ii+1].geometry().asPoint()]
    # Add a new feature and assign the geometry
    feat = QgsFeature()
    feat.setGeometry(QgsGeometry.fromPolyline(fl_polyline))
    prov.addFeatures([feat])
